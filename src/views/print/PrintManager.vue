      ...
      <div class="space-y-6">
        ...
        <div class="grid grid-cols-3 gap-4">
          <div
            v-for="stat in stats"
            :key="stat.label"
            class="p-4 rounded-lg border transition-colors bg-white border-gray-200 hover:border-gray-300"
          >
        ...
        <!-- 快捷操作 -->
        <div class="text-lg font-semibold text-blue-600">快捷操作</div>

        <div class="grid grid-cols-2 gap-4">
          <div
            v-for="action in quickActions"
            :key="action.title"
            class="p-6 rounded-lg border cursor-pointer transition-all hover:border-blue-500/50 bg-white border-gray-200"
            @click="router.push(action.path)"
          >
            <div class="flex flex-col items-center text-center">
              <div class="w-12 h-12 rounded-xl flex items-center justify-center mb-3 bg-blue-100 text-blue-500">
                <NIcon size="24">
                  <action.icon />
                </NIcon>
              </div>
              <div class="font-semibold">{{ action.title }}</div>
              <div class="text-sm mt-1 text-gray-500">{{ action.desc }}</div>
            </div>
          </div>
        </div>

        <!-- 支持格式 -->
        <div class="text-lg font-semibold text-blue-600">支持格式</div>
    ...
    <template #output>
      <div class="space-y-4">
        <div class="flex items-center justify-between">
          <div class="text-lg font-semibold text-blue-600">最近打印</div>
